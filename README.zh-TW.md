...
> 剛接觸嗎？**[5 分鐘快速上手 →](docs/QUICKSTART.md)** 帶你走過每一項功能（英文）。
...
- **存成或分享為單一檔案** —— 整個工作階段（音訊＋逐字稿＋摘要＋語者＋封面）打包成一個 **`.ogg`** 或 **`.m4a`**，**在任何播放器都能播**（並顯示標題、封面與作為歌詞的逐字稿），**用 VoxSum 開啟時所有內容也完整保留**。想要最廣的相容性（iPhone、車機、各種播放器）就選 `.m4a`。
