...
- **存成或分享為單一檔案** —— 整個工作階段（音訊＋逐字稿＋摘要＋語者＋封面）打包成一個 **`.ogg`** 或 **`.m4a`**，**在任何播放器都能播**（並顯示標題、封面、摘要與逐字稿——見[*Android 音樂播放器中的同步歌詞*](#android-音樂播放器中的同步歌詞)），**用 VoxSum 開啟時所有內容也完整保留**。想要最廣的相容性（iPhone、車機、各種播放器）就選 `.m4a`。
...
（不支援同步的播放器會顯示文字，但會看到 `[mm:ss]` 時間軸。）建議用 **`.m4a`**——它的歌詞標籤比 `.ogg` 更廣為支援。

| 程式 | 在哪裡打開歌詞 | 即時同步 |
|---|---|---|
| **Retro Music** *(免費)* | 播放畫面歌詞（歌詞） | ✅ 已驗證 |
| **Gramophone** *(免費)* | 歌詞檢視 | ✅ 已驗證 |
| **Poweramp** | 歌詞檢視 | 🔄 預期支援 |
| **Musicolet** *(免費)* | **歌詞**分頁 | 🔄 預期支援 |
| **Oto Music** *(免費)* | 歌詞檢視 | 🔄 預期支援 |

> **備註。** ✅＝已在 Pixel 驗證；🔄＝有文件記載支援同步歌詞。**摘要**存在標準的**註解（comment）**標籤。
