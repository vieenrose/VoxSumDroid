...
internal object SpectralClustering {
    ...
    fun cluster(embs: Array<FloatArray>, numClusters: Int = -1, maxSpeakers: Int = 8): IntArray {
        ...
        // Diagnostic: k=1 on audio a listener hears as multi-speaker is the single most-reported
        // diarization failure, and from the outside it is indistinguishable from "the model thinks
        // it is one voice". The eigenvalues are what actually decided, so log them.
        // println, not android.util.Log: Log is unmocked in JVM unit tests and threw
        // "Method i in android.util.Log not mocked", failing five clustering tests. On device
        // stdout still reaches logcat.
