        ...
        fun create(
            ...
            val ctxIds = if (context.isBlank() || mergesTxt == null || !mergesTxt.exists()) {
                ...
                runCatching {
                    ...
                    .onFailure { android.util.Log.w("voxsum-moss", "context biasing unavailable", it) }
                    .getOrDefault(IntArray(0))
            }
            if (ctxIds.isNotEmpty()) android.util.Log.i("voxsum-moss", "context biasing: ${ctxIds.size} tokens")
            cacheDir?.mkdirs()
            val c = nativeInit(
