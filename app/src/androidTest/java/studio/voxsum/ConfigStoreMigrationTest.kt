 ...
 * instrumented test (the JVM SummaryLanguageTest covers the pure routing table).
 */
...
class ConfigStoreMigrationTest {
    ...
    @Test fun legacyTraditionalChineseTrueMigratesToZhHant() {
        ...
        assertEquals("zh-Hant", ConfigStore.load(ctx).summaryLanguage)
    }

    @Test fun legacyTraditionalChineseFalseMigratesToAuto() {
        prefs().edit().putBoolean("traditionalChinese", false).commit()
        assertEquals("auto", ConfigStore.load(ctx).summaryLanguage)
    }

    @Test fun savedSummaryLanguageWinsOverLegacyBoolean() {
        prefs().edit().putString("summaryLanguage", "ja").putBoolean("traditionalChinese", true).commit()
        assertEquals("ja", ConfigStore.load(ctx).summaryLanguage)
    }
    ...
    @Test fun freshInstallDefaultsToDeviceLanguage() {
        ...
        assertEquals(SummaryLanguage.defaultFor(ctx).id, ConfigStore.load(ctx).summaryLanguage)
    }
}
