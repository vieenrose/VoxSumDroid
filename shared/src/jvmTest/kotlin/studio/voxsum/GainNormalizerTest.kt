...
class GainNormalizerTest {
    ...
    @Test
    fun silentLeadThenQuietSpeechStillBoosted() {
        // A meeting that opens with 40 s of room tone before anyone speaks (the ES2004a case):
        // the adaptive decision must WAIT for voiced material instead of deciding on silence.
        val silence = FloatArray(40 * sr)
        val speech = burstySine(30, amp = 0.0045f)
        val input = silence + speech
        val (out, gain) = run(input)
        assertEquals(input.size, out.size)
        assertTrue("silent lead defeated the boost (gain x$gain)", gain > 10f)
        val i = silence.size + 400   // inside the first burst
        assertEquals(input[i] * gain, out[i], 1e-6f)
    }

    @Test
    fun tailCarriesTheSameGainAsTheLead() {
        // 90 s quiet stream: the decision fires once ≈10 s of voiced frames are seen; the tail
        // must be scaled by the SAME constant gain (one gain per file — no dynamics).
