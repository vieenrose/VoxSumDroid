...
class NemotronLiteAsr(
    ...
    // nativeInit already handed back a live handle, and a missing .so raises UnsatisfiedLinkError).
    // Either way the ~600 MB engine above is already allocated and must not leak — an escaping
    // exception from a property initializer means the caller's use{} block never runs.
    private val vad = try {
    ...
    override fun decodeSlice(samples: FloatArray): String {
        ...
        return try {
            // The encoder is fixed at MAX_DECODE_SEC and silently truncates anything longer, so
            // split here too — drain() does, and this path is reachable with utterances produced
            // by a 30 s-ceiling backend (re-diarize over a restored session).
            ...
            logWarn(
                "split re-decode failed; keeping the fused line", t)
            ""
        }
    }
    ...
    companion object {
        ...
        // Desktop build: no android.util.Log. Keep the same tag shape so logs read
        // the same on both platforms.
        private fun logInfo(msg: String) = System.err.println("I/NemotronLiteAsr: $msg")
        private fun logWarn(msg: String, t: Throwable? = null) {
            System.err.println("W/NemotronLiteAsr: $msg")
            t?.printStackTrace()
        }
    }
}
